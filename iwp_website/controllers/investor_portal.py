...
class InvestorPortal(CustomerPortal):
    ...
    def my_wallet_share(self, **kw):
        ...
        if "delete_share_success" in request.session:
            ...
            values["delete_share_success"] = request.session["delete_share_success"]
        ...
        return request.render("iwp_website.portal_my_wallet_share", values)
    ...
    def my_history_share(self, sortby=None, **kw):
        ...
        return request.render("iwp_website.portal_my_history_share", values)
    ...
    def my_wallet_loan(self, sortby=None, **kw):
        ...
        data = []
        WalletLine = namedtuple("WalletLine", ["structure", "total_amount", "lines"])
        ...
        for (structure, loanlines) in grouped_lil:
            ...
            data.append(
                WalletLine(
                    structure=structure, total_amount=total_amount, lines=lines.sudo()
                )
            )
        ...
        if "delete_loan_success" in request.session:
            ...
            values["delete_loan_success"] = request.session["delete_loan_success"]
        ...
        return request.render("iwp_website.portal_my_wallet_loan", values)
    ...
    def my_history_loan(self, sortby=None, **kw):
        ...
        return request.render("iwp_website.portal_my_history_loan", values)
    ...
    def structures(self, page=1, sortby=None, **kw):
        ...
        for struct in structures:
            data.append(
                StructureLine(
                    ...
                    display_share_action=self.display_share_action(user, struct),
                    display_loan_action=self.display_loan_action(user, struct),
                )
            )
        ...
        return request.render("iwp_website.structures", values)
    ...
    def account(self, redirect=None, **post):
        ...
        if request.httprequest.method == "POST":
            form = self.user_form(data=request.params, context={"user": user})
            if form.is_valid():
                if is_company:
        ...
        if is_company:
            return request.render("iwp_website.investor_company_details", qcontext)
            ...
            initial.update(
                {
                    ...
                    "birthdate": (
                        user.birthdate_date.isoformat() if user.birthdate_date else ""
                    ),
                    "phone": user.phone,
                    "lang": user.lang,
                    "street": user.street,
                    "zip_code": user.zip,
                    "city": user.city,
                    "country": (str(user.country_id.id) if user.country_id else ""),
                }
            )
            if user.bank_ids:
            ...
            initial.update(
                {
                    ...
                    "city": company.city,
                    "country": (company.country_id and str(company.country_id.id)),
    ...
    def display_share_action(self, user, structure):
        ...
        return bool(
            [
                share
                for share in structure.share_type_ids
                if (
                    share.state != "close"
    ...
    def display_loan_action(self, user, structure):
        ...
        return bool(
            [
                loan
                for loan in structure.loan_issue_ids
                if (
                    loan.state == "ongoing"
