...
class ManualLoanAction(http.Controller):
    ...
    def new_manual_loan(self, struct_id=None, **params):
        ...
        if request.httprequest.method == "POST":
            form = self.manual_loan_form(data=request.params, context=form_context)
            if form.is_valid():
    ...
    def delete_manual_loan(self, loan_line_id=None, **params):
        ...
        loanline = request.env["loan.issue.line"].sudo().browse(loan_line_id).exists()
    ...
    def manual_loan_form_initial(self, context=None):
        ...
        if struct:
            ...
            default_loan_issue = default_loan_issues[0] if default_loan_issues else None
    ...
    def manual_loan_form_processing(self, form, context=None):
        ...
        loan_line_mgr.sudo().create(self.loan_line_vals(form, context=context))
